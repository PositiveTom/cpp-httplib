...
using Headers = std::multimap<std::string, std::string, detail::ci>;/*detail::ci是自定义的排序规则*/
...
struct Request {
  ...
  // for server
  std::string version;/*version method,target存储请求行，重点！！！*/
  std::string target;
  Params params;
...
inline bool stream_line_reader::getline() {/*一次读一行*/
  fixed_buffer_used_size_ = 0;
  glowable_buffer_.clear();

  for (size_t i = 0;; i++) {
    char byte;
    auto n = strm_.read(&byte, 1);/*一次读一个字节*/
...
inline void stream_line_reader::append(char c) {
  if (fixed_buffer_used_size_ < fixed_buffer_size_ - 1) {
    ...
    if (glowable_buffer_.empty()) {
      ...
      glowable_buffer_.assign(fixed_buffer_, fixed_buffer_used_size_);/*把fixed_buffer_中的fixed_buffer_used_size_个字符赋值给string对象*/
    }
    glowable_buffer_ += c;
  }
}
...
inline ssize_t read_socket(socket_t sock, void *ptr, size_t size, int flags) {
  return handle_EINTR([&]() {/*阻塞IO读取的通用写法*/
    return recv(sock,
#ifdef _WIN32
...
process_server_socket_core(const std::atomic<socket_t> &svr_sock, socket_t sock,
                           ...
                           time_t keep_alive_timeout_sec, T callback) {/*这里嵌套的lambda表达式代码显得很鸡肋*/
...
inline void get_remote_ip_and_port(socket_t sock, std::string &ip, int &port) {
  ...
  if (!getpeername(sock, reinterpret_cast<struct sockaddr *>(&addr),
                   &addr_len)) {/*获取与某个套接字关联的外地协议地址*/
#ifndef _WIN32
    if (addr.ss_family == AF_UNIX) {/*这里有什么用?*/
...
#endif
    get_ip_and_port(addr, addr_len, ip, port);/*从addr中解析出ip地址和端口号*/
  }
}
...
inline ssize_t SocketStream::read(char *ptr, size_t size) {
  ...
  if (!is_readable()) { return -1; }/*通过select的方式来检测服务器端口是否有数据到来*/

  read_buff_off_ = 0;
  read_buff_content_size_ = 0;

  if (size < read_buff_size_) {/*如果读取的字节数 小于 缓冲区容量*/
    auto n = read_socket(sock_, read_buff_.data(), read_buff_size_,
                         CPPHTTPLIB_RECV_FLAGS);/*一次读取缓冲区容量大小的字节数,返回读取的字节数*/
    if (n <= 0) {
      return n;
    } else if (n <= static_cast<ssize_t>(size)) {/*如果读取出的字节数, 小于等于期望读取的字节数*/
...
#endif
  /*解析字符串并提取静态片段和路径参数, 例如 : /2023/07/27/happy:birthday/chatgpt", /2023/07/27/happy给static_fragments_, birthday/chatgpt给param_names_*/
  while (true) {
    const auto marker_pos = pattern.find(marker, last_param_end);/*从字符串last_param_end位置开始,查找字符串marker，返回该字符串首次出现的索引*/
    if (marker_pos == std::string::npos) { break; } /*如果没有找到*/

    static_fragments_.push_back(/*如果找到了,*/
...
/*获得http报文的请求行version method target*/
inline bool Server::parse_request_line(const char *s, Request &req) {
...

  {
    ...
    detail::split(s, s + len, ' ', [&](const char *b, const char *e) {/*把GET / HTTP/1.1 拆分开, 以' '为分隔符*/
      switch (count) {
...
inline bool Server::listen_internal() {
...

  {
    ...
    while (svr_sock_ != INVALID_SOCKET) {
      ...
      task_queue->enqueue([this, sock]() { process_and_close_socket(sock); });/*任务队列添加任务之后,线程池就自己开始执行了！！*/
    }
...
Server::process_request(Stream &strm, bool close_connection,
                        bool &connection_closed,
                        const std::function<void(Request &)> &setup_request) {
  std::array<char, 2048> buf{};/*固定大小的数组,c++11新引入的*/
  ...
  res.headers = default_headers_;/*一个multimap容器*/
  ...
  // Check if the request URI doesn't exceed the limit /*检查请求URL是否超过限制, GET URL HTTP/1.1*/
  ...
  if (!parse_request_line(line_reader.ptr(), req) ||
      !detail::read_headers(strm, req.headers)) {/*如果没有获取到正确的请求行和首部字段,返回400*/
    res.status = 400;
    return write_response(strm, close_connection, req, res);
  }
  ...
  strm.get_remote_ip_and_port(req.remote_addr, req.remote_port);/*获取客户端的ip地址和端口*/
  req.set_header("REMOTE_ADDR", req.remote_addr);
  req.set_header("REMOTE_PORT", std::to_string(req.remote_port));

  strm.get_local_ip_and_port(req.local_addr, req.local_port);/*获取服务器的ip地址和端口*/
  req.set_header("LOCAL_ADDR", req.local_addr);
  req.set_header("LOCAL_PORT", std::to_string(req.local_port));

  if (req.has_header("Range")) {/*GET报文没有这个*/
  ...
  if (setup_request) { setup_request(req); }/*GET报文没有这个*/

  if (req.get_header_value("Expect") == "100-continue") {/*GET报文没有这个*/
    auto status = 100;
...
#else
  try {
    routed = routing(req, res, strm);/*GET报文运行这里,有异常处理机制*/
